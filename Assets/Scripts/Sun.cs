...
public class Sun : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        InitAsIcosohedron();
        Subdivide(3);
        GenerateMesh("Sun Mesh", m_Material);
        sunSize = Random.Range(5, 8);
        this.transform.localScale = new Vector3(sunSize,sunSize,sunSize);
    }
    ...
    public void GenerateMesh(string name, Material material)
    {
        if (m_SunMesh)
            Destroy(m_SunMesh);

        m_SunMesh = new GameObject(name);

        MeshRenderer surfaceRenderer = m_SunMesh.AddComponent<MeshRenderer>();
        ...
        for (int i = 0; i < m_Polygons.Count; i++)
        {
            ...
            Color32 polyColor = Color32.Lerp(Color.yellow, Color.red, Random.Range(0.0f, 1.0f)); //different sun types later with diff colors?
        ...
        MeshFilter terrainFilter = m_SunMesh.AddComponent<MeshFilter>();
        terrainFilter.mesh = terrainMesh;
        m_SunMesh.transform.parent = this.transform;
    }
}